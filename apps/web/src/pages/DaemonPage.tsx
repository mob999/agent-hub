...
export function DaemonPage({ devices, deviceError, onDevicesChanged }: DaemonPageProps) {
  ...
  useEffect(() => {
    ...
    void desktopDaemon.getStatus().then((status) => {
      if (mounted && status.deviceId) {
        setSelectedDeviceId((current) => current ?? status.deviceId ?? null)
      }
    })
  ...
  const openReconnectModal = () => {
    if (!selectedDevice || selectedDeviceIsDesktopManaged) {
      return
    }
  ...
  const deleteDevice = async () => {
    if (!selectedDevice || deleteLoading || selectedDeviceIsDesktopManaged) {
      return
    }
                ...
                <div className="grid max-w-xl gap-4">
                  ...
                  <div className="grid border-t border-[#eef0f3]">
                    ...
                    <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-3 border-b border-[#eef0f3] py-3">
                      ...
                      <button
                        type="button"
                        className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-[#f1d27a] bg-[#fff8df] text-[#b28600] transition hover:border-[#d7af2f] hover:bg-[#fff1bf] disabled:cursor-not-allowed disabled:border-[#dde1e6] disabled:bg-[#f4f4f4] disabled:text-[#a2a9b0]"
                        disabled={selectedDeviceIsDesktopManaged}
                        onClick={openReconnectModal}
                        title={selectedDeviceIsDesktopManaged ? t('daemon.localReconnectDisabled') : t('common.reconnect')}
                        aria-label={selectedDeviceIsDesktopManaged ? t('daemon.localReconnectDisabled') : t('daemon.reconnectDevice')}
                      >
                        <Renew size={16} />
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-3 py-3">
                      ...
                      <button
                        type="button"
                        className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-[#ffd7d9] bg-[#fff1f1] text-[#da1e28] transition hover:border-[#ffb3b8] hover:bg-[#ffe0e2] disabled:cursor-not-allowed disabled:border-[#dde1e6] disabled:bg-[#f4f4f4] disabled:text-[#a2a9b0]"
                        disabled={deleteLoading || selectedDeviceIsDesktopManaged}
                        onClick={() => {
                          void deleteDevice()
                        }}
                        title={
                          selectedDeviceIsDesktopManaged
                            ? t('daemon.localDeleteDisabled')
                            : deleteConfirming
                              ? t('daemon.deleteConfirm')
                              : t('daemon.delete')
                        }
                        aria-label={
                          selectedDeviceIsDesktopManaged
                            ? t('daemon.localDeleteDisabled')
                            : deleteConfirming
                              ? t('daemon.deleteConfirmDevice')
                              : t('daemon.deleteDevice')
                        }
                      >
                        {deleteLoading ? (
                          <span
...
function isDesktopManagedDaemonDevice(device: DaemonDevice | null): boolean {
  return device?.id.startsWith('desktop-') ?? false
}
