...
Agent Hub 面向的核心用户是需要借助 AI Agent 完成工程、内容或自动化任务的普通用户。用户在系统中既是会话发起者，也是任务确认者和产物使用者。他们需要能够创建不同类型的会话，向一个或多个 Agent 发送任务，查看任务执行过程，并对生成的文件、网页预览、项目变更或部署结果进行后续处理。因此，普通用户侧的需求不仅是“获得回答”，还包括持续追踪任务、理解运行状态、管理产物和恢复历史上下文。

第二类角色是 Agent 或 runtime。Agent 是用户在工作台中看到和交互的协作成员，runtime 则是实际执行任务的底层能力来源。当前系统主要面向 Claude Code、Codex 等本地 CLI runtime，也为后续接入云端 Agent 或自定义 runtime 保留扩展空间。Agent/runtime 的职责是接收经过授权的任务，调用本地或远程工具完成执行，并把消息、日志、任务状态和 Artifact 回传给平台。

第三类角色是系统服务，包括 API、Worker、数据库、Redis、对象存储和实时通道等基础组件。系统服务不直接表现为聊天成员，但负责支撑整个平台的控制面和执行面能力：API 管理用户、权限、会话、Agent 和元数据；Worker 负责长任务消费和运行事件写入；数据库与对象存储保存业务状态和生成产物；实时通道负责把运行过程同步给客户端。

需要特别说明的是，UI 客户端不是 Agent 执行者。Web 和桌面端主要负责交互、展示和用户确认，不承担长任务执行、全局权限判断或聊天历史的权威存储。同样，daemon 也不是第二套后端，它只是在用户本机运行的轻量执行器，负责连接本地 runtime 并执行经过平台授权的任务。

### 2.2 功能需求

在认证与用户会话方面，系统需要支持基于 GitHub OAuth 的登录方式，建立稳定的用户身份和会话状态。用户登录后应能够进入工作台，查看自己的会话、Agent、执行器和历史产物。系统还需要支持用户资料更新、登录状态保持和退出登录，保证不同用户之间的数据隔离。

在会话与 Agent 协作方面，系统需要提供单 Agent 私聊、群聊和项目会话等不同协作空间。用户可以在会话中发送消息，也可以通过 @ 的方式指定 Agent 参与任务。对于涉及多个 Agent 的任务，系统应支持 orchestrator 进行目标拆分和任务分派，使规划、执行、验证等步骤能够在同一个聊天上下文中组织起来。

在 Agent 管理方面，用户需要能够创建、编辑、归档和恢复 Agent，并将 Agent 与可用 runtime 进行绑定。Agent 应具备名称、头像、说明、运行环境和在线状态等基本信息，使用户能够理解每个 Agent 的能力边界。系统还需要根据 daemon 上报的 runtime 状态判断 Agent 是否可用，避免用户把任务分配给不可执行的目标。

在 Run 与任务状态方面，系统应将用户消息转化为可追踪的执行单元。Run 需要具备排队、运行、成功、失败、取消、中断等状态，并能够记录执行过程中的日志、事件和中间结果。前端需要展示这些状态，使用户知道任务是否已经开始、是否仍在运行、失败原因是什么，以及是否产生了新的回复或产物。

在 Artifact 与部署方面，系统需要支持 Agent 生成文件、项目变更、网页预览和 deployment 记录。用户应能够在聊天流或 Artifact 工作区中查看这些产物，并在需要时继续编辑、发布或打开预览链接。对于静态网页等可发布产物，系统应记录部署结果，使生成内容从一次性输出变成可访问、可追踪的成果。

在 daemon 与桌面端方面，系统需要支持本地执行器注册、设备在线状态展示和 runtime 检测。浏览器端应保留手动连接 daemon 的能力，桌面端则进一步提供托管本地 daemon 的能力，减少用户复制命令和维护后台进程的成本。桌面端与 Web 端应共享主要业务界面，避免形成两套割裂的产品体验。

在实时反馈方面，系统需要将消息、任务、RunEvent、Artifact 和执行器状态及时同步到客户端。用户在等待 Agent 执行时，应能够看到排队、运行、输出增量、失败和完成等变化；在收到新消息或后台事件时，也应有明确的前端提醒和状态更新。

### 2.3 非功能需求

工程可靠性是本系统最重要的非功能需求。Agent 任务往往运行时间较长，不能依赖一次 HTTP 请求完成。因此，系统必须将用户请求、任务排队、任务执行和事件回传拆分开来：API 负责快速创建 Run 并返回，Worker 在后台消费任务，daemon 或 runtime 负责具体执行。这样既能保证控制面响应稳定，也能为任务重试、取消、失败记录和运行追踪提供基础。

本地执行器需要具备清晰的安全边界。用户机器上的代码、工具和凭据不应直接暴露给公网服务，因此 daemon 必须以出站连接方式接入 Worker gateway，而不是要求用户开放本地端口。所有本地任务都应基于平台授权的设备 token 和 workspace root 执行，避免把 daemon 设计成无边界的本地后门。

部署形态需要支持多服务分离。Web 前端应保持纯静态 SPA，便于部署到 Vercel 等静态托管平台；API 和 Worker 应作为独立服务部署，分别承担控制面和执行面职责；数据库、Redis 和对象存储也应独立管理。尤其是生成产物不能依赖某一个容器的本地磁盘，否则在生产多容器环境中会出现文件丢失或 API/Worker 无法共享的问题。

状态可观测性和可恢复性也是系统稳定运行的基础。Run 状态、运行事件、消息、Artifact 元数据和 deployment 记录都需要持久化，使用户能够在刷新页面、切换设备或任务结束后继续查看历史结果。对于缓存、实时推送和后台任务失败，系统应保留数据库中的权威状态，并通过重新拉取或重试恢复前端显示。

前端体验层面，系统需要提供清晰的 loading、empty、error 和实时更新反馈。由于 Agent 任务具有不确定耗时，用户必须能够理解当前是否正在排队、执行、失败或已经完成。对于消息发送、会话切换、任务状态聚合和 Artifact 加载等高频操作，前端还需要通过缓存和乐观更新减少等待感。

最后，系统需要具备可测试和可维护性。共享协议、状态转换、缓存、对象存储、认证和 daemon gateway 等关键逻辑应能够通过单元测试或集成测试验证。API route、后端领域逻辑和 browser-safe 协议类型应保持清晰边界，使后续功能扩展不会破坏已有运行链路。
